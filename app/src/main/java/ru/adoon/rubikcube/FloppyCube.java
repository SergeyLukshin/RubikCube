...
public class FloppyCube {
    public volatile ArrayList<ArrayList<ArrayList<FloppyCubeItem>>> mItems = null;//new ArrayList<ArrayList<ArrayList<CubeItemOld>>>();
    ...
    FloppyCube(Context context) {

        mItems = new ArrayList<ArrayList<ArrayList<FloppyCubeItem>>>();
    ...
    public synchronized void CubeInit(int CubeDimX, int CubeDimY, int CubeDimZ) {
        ...
        if (mItems.size() == 0) {
            for (int x = 0; x <= mCubeDimX - 1; x++) {
                mItems.add(new ArrayList<ArrayList<FloppyCubeItem>>());
                for (int y = 0; y <= mCubeDimY - 1; y++) {
                    mItems.get(x).add(new ArrayList<FloppyCubeItem>());
                    for (int z = 0; z <= mCubeDimZ - 1; z++) {
                        if (y != (mCubeDimY - 1) / 2)
                            mItems.get(x).get(y).add(new FloppyCubeItem(x/* - 1*/, y/* - 1*/, z/* - 1*/, null, mCubeDimX, mCubeDimY, mCubeDimZ, mScale, false));
                        else
                            mItems.get(x).get(y).add(new FloppyCubeItem(x/* - 1*/, y/* - 1*/, z/* - 1*/, null, mCubeDimX, mCubeDimY, mCubeDimZ, mScale, true));
                    }
                }
            }
        }
            ...
            for (int x = 0; x <= mCubeDimX - 1; x++) {
                ...
                for (int y = 0; y <= mCubeDimY - 1; y++) {
                    ...
                    for (int z = 0; z <= mCubeDimZ - 1; z++) {
                        if (y != (mCubeDimY - 1) / 2)
                            mItems.get(x).get(y).set(z, new FloppyCubeItem(x/* - 1*/, y/* - 1*/, z/* - 1*/, null, mCubeDimX, mCubeDimY, mCubeDimZ, mScale, false));
                        else
                            mItems.get(x).get(y).set(z, new FloppyCubeItem(x/* - 1*/, y/* - 1*/, z/* - 1*/, null, mCubeDimX, mCubeDimY, mCubeDimZ, mScale, true));
                    }
                }
            }
        }
        ...
        for (int x = 0; x < mItems.size(); x++) {
            for (int y = 0; y < mItems.get(x).size(); y++) {
                for (int z = 0; z < mItems.get(x).get(y).size(); z++) {
                    FloppyCubeItem ci = mItems.get(x).get(y).get(z);
                    if (ci != null) {
                        ci.InitMatrix();
                        if (a != null) {
    ...
    public boolean StoreItemPosition(Action a) {
        ...
        if (isEqualDim()) {
            ...
            if (a.m_ActionAxisRotate == Structures.AXE_X) x = a.m_ActionPosRotate;
            if (a.m_ActionAxisRotate == Structures.AXE_Y) y = a.m_ActionPosRotate;
            if (a.m_ActionAxisRotate == Structures.AXE_Z) z = a.m_ActionPosRotate;

            FloppyCubeItem ci = mItems.get(x).get(y).get(z);

            if (ci != null) {

                boolean bVisible = ci.m_bVisible;

                int[] verge_color_indexes_ = ci.verge_color_index.clone();
                FloppyCubeItem.Rotate(verge_color_indexes_, a.m_ActionAxisRotate, a.m_ActionDirectRotate, isEqualDim());
                mItems.get(x).get(y).set(z, new FloppyCubeItem(x, y, z, verge_color_indexes_, mCubeDimX, mCubeDimY, mCubeDimZ, mScale, bVisible));
            }
        }
    ...
    public Action GetAction(int[] item, float[] start, float[] end/*, boolean bSelect*/)
    {
        ...
        FloppyCubeItem pi = mItems.get(item[0]).get(item[1]).get(item[2]);
        int vertex = item[3] / 2;
            ...
            for (int y = 0; y < mItems.get(x).size(); y++) {
                for (int z = 0; z < mItems.get(x).get(y).size(); z++) {
                    ...
                    if (ci != null) {
                        ...
                        ed.putString("FloppyCubeVertexColor" + String.valueOf(mCubeDimX) + String.valueOf(mCubeDimY) + String.valueOf(mCubeDimZ) + String.valueOf(x) + String.valueOf(y) + String.valueOf(z), str);
                        ed.putBoolean("FloppyCubeVisible" + String.valueOf(mCubeDimX) + String.valueOf(mCubeDimY) + String.valueOf(mCubeDimZ) + String.valueOf(x) + String.valueOf(y) + String.valueOf(z), ci.m_bVisible);
                    }
                }
            }
        }
        ...
        for (int x = 0; x < mItems.size(); x++) {
            for (int y = 0; y < mItems.get(x).size(); y++) {
                for (int z = 0; z < mItems.get(x).get(y).size(); z++) {
                    FloppyCubeItem ci = mItems.get(x).get(y).get(z);
                    if (ci != null) {
                        String str = sPref.getString("FloppyCubeVertexColor" + String.valueOf(mCubeDimX) + String.valueOf(mCubeDimY) + String.valueOf(mCubeDimZ) + String.valueOf(x) + String.valueOf(y) + String.valueOf(z), "");
                        boolean bVisible = sPref.getBoolean("FloppyCubeVisible" + String.valueOf(mCubeDimX) + String.valueOf(mCubeDimY) + String.valueOf(mCubeDimZ) + String.valueOf(x) + String.valueOf(y) + String.valueOf(z), true);
                        if (str != "") {
                            ...
                            mItems.get(x).get(y).set(z, new FloppyCubeItem(x/* - 1*/, y/* - 1*/, z/* - 1*/, verge_color_indexes_, mCubeDimX, mCubeDimY, mCubeDimZ, mScale, bVisible));
                        }
                    }
                }
            }
